...
(function() {
  ...
  function hasAcademicMetadata() {
    ...
    const articleKeywords = [
      ...
      "/pubmed", "/pmc", "/journal", "/pii/", "/fulltext", "/download"
    ];
    ...
    return hasArticleKeyword || hasDoiInUrl || hasAbstractEl;
  }
    ...
    chrome.runtime.sendMessage({
      ...
      if (response && response.success && response.data) {
        ...
        chrome.storage.local.get(["auto_redirect", "pdf_landing_cache"], (config) => {
          if (config.auto_redirect && paperMeta.pdfUrl) {
  ...
  function extractPageMetadata() {
    ...
    if (!journal) {
      ...
      else if (host.includes("sciencedirect.com")) journal = "ScienceDirect";
      else if (host.includes("wiley.com")) journal = "Wiley";
      else if (host.includes("springer.com")) journal = "Springer";
      else if (host.includes("ieee.org")) journal = "IEEE Xplore";
      else if (host.includes("arxiv.org")) journal = "arXiv";
      else if (host.includes("biorxiv.org")) journal = "bioRxiv";
    }
  ...
  function deduceLandingPage(url, doi, title, cachedLandingUrl) {
    ...
    // Rule 4: bioRxiv/medRxiv PDF back-resolve
    ...
    if (host.includes("elsevier.com") && href.includes("/reader/sd/pii/")) {
      const match = url.match(/\/pii\/([sS]\d{15}\d*|[sS]?\d{4}-\d{3}[\dX]\(\d\d\)\d{5}-\d)/);
      if (match) {
        return `https://www.sciencedirect.com/science/article/pii/${match[1]}`;
      }
      const generalPii = url.match(/\/pii\/([a-zA-Z0-9]+)/);
      if (generalPii) {
        return `https://www.sciencedirect.com/science/article/pii/${generalPii[1]}`;
      }
    }
